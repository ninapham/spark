...
def tokenize(string):
    ...
    """
    return filter(lambda x: x not in stopwords, list(simpleTokenize(string)))
...
def countTokens(vendorRDD):
    ...
    """
    return vendorRDD.map(lambda x: x[1]).map(lambda x: len(x)).sum()
...
def findBiggestRecord(vendorRDD):
    ...
    """
    return vendorRDD.map(lambda x: (x[0], x[1])).takeOrdered(1, key=lambda x: -len(x[1]))
...
def tf(tokens):
    ...
    for word in dictionary:
        dictionary[word] = dictionary[word]/float(len(tokens))
...
def idfs(corpus):
    ...
    """
    N = corpus.count()
    uniqueTokens = corpus.map(lambda x: set(x[1])).flatMap(lambda x: x)
...
def tfidf(tokens, idfs):
    ...
    for key in tfs:
        if key in idfs.keys():
            tfIdfDict[key] = idfs[key]*tfs[key]
    return tfIdfDict
...
avgSimDups = (trueDupsRDD.map(lambda x: x[1]).map(lambda x: x[0]).sum())/float(trueDupsCount)

nonDupsRDD = (sims.leftOuterJoin(goldStandard).filter(lambda x: x[1][1]!='gold'))

avgSimNon = (nonDupsRDD.map(lambda x: x[1]).map(lambda x: x[0]).sum())/float(nonDupsRDD.count())
...
#googleWeightsRDD = tfidf(googleFullRecToToken, idfsFullWeights)
...
#                .join(googleInvPairsRDD).map(lambda x: swap(x)).reduceByKey(lambda a,b: a+b)).cache()

commonTokens = (amazonInvPairsRDD.join(googleInvPairsRDD)
                .map(swap).groupByKey().mapValues(lambda x : list(x))
                .cache())
