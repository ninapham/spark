...
dayAndHostTuple = access_logs.map(lambda x: (x.date_time.day,1)).reduceByKey(lambda a,b: a+b).distinct()

groupedByDay = dayAndHostTuple.join(dailyHosts)

sortedByDay = groupedByDay.sortByKey()
sortedByDay.take(5)
avgDailyReqPerHost = (sortedByDay.map(lambda (a, (b, c)): (a, b/c))).cache()
