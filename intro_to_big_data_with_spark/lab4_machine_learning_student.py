...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    ratings_tuple = IDandRatingsTuple[1]
    num_ratings = len(ratings_tuple)
    avg_rating = float(sum(ratings_tuple))/len(ratings_tuple)
...
# Use the Python list myRatedMovies to transform the moviesRDD into an RDD with entries that are 
# pairs of the form (myUserID, Movie ID) and that does not contain any movies that you have rated.
...
#an RDD with tuples of the form (Movie ID, (Predicted Rating, number of ratings))
...
#with tuples of the form (Predicted Rating, Movie Name, number of ratings), for movies with more than 75 ratings
ratingsWithNamesRDD = predictedWithCountsRDD.filter(lambda x: x[1][1]>75).join(moviesRDD).map(lambda x: (x[1][0][0], x[1][1], x[1][0][1]))
