...
def calcRMSE(labelsAndPreds):
    ...
    """
    return (labelsAndPreds.map(lambda (label,prediction) : squaredError(label,prediction)).reduce(lambda x,y : x+y)/labelsAndPreds.count())**.5

labelsAndPreds = sc.parallelize([(3., 1.), (1., 2.), (2., 2.)])
#RMSE = ([((3-1)^2 + (1-2)^2 + (2-2)^2) / 3])**.5 = 1.291
...
def linregGradientDescent(trainData, numIters):
    ...
    for i in range(numIters):
        ...
        gradient = exampleData.map(lambda x: gradientSummand(w, x)).sum()
...
def twoWayInteractions(lp):
    ...
    finalfeatures = np.hstack((lp,newList))
    return (lp.label,finalfeatures) 
...
modelInteract = LinearRegressionWithSGD.train(trainDataInteract, numIters, alpha,
                                              miniBatchFrac, regParam=reg,
                                              regType='l2', intercept=True)
labelsAndPredsInteract = valDataInteract.map(lambda lp: (lp.label, valDataInteract.predict(lp.features)))
...
# TODO: Replace <FILL IN> with appropriate code
labelsAndPredsTest = parsedTestData.map(lambda x: (x.label, modelInteract.predict(x.features)))
